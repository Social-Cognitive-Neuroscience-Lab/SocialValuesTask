---
title: "Qualtrics Scoring (in person testing)"
output:
  html_document:
    df_print: paged
---
...
# Import data
```{r}
d_raw <- read_xlsx('../../scratch-foo/data/moral_choice/inperson_3_con.xlsx',
               skip=1, na = c(NA, 'NA'))
d_raw <- d_raw %>%
...
print(str_c(nrow(d_real)," subjects"))
...
d_view <- d_support %>%
  ...
  cluster_scale(Support, SID) %>% 
  cluster_scale(Moral, SID)
...
d %>% select(Subj, everything()) %>% write_csv('ChoiceData_con.csv')
d %>% select(Subj, everything()) %>% 
  filter(RT < 60000 & RT > 150) %>% 
  write_csv('ChoiceData_trim_con.csv')
# d %>% 
...
d %>% 
  ...
  filter(Trial > 30) %>% 
  select(subj_idx, rt, response, support, moral) %>%
  write_csv('choice_hddm_late_con.csv')
...
d %>% 
  ...
  select(subj_idx, rt, response, support, moral) %>%
  filter(support != 0) %>%
  write_csv('choice_hddm_unambig_con.csv')
...
```{r}
d_trim %>% 
  ggplot(aes(Support_bin, RTs, color=Subject)) + 
  geom_point(alpha=.2) + 
...
```{r}
d_trim %>% 
  ggplot(aes(LeftMoral_diff, RTs, color=Subject)) + 
  geom_point(alpha=.2) + 
  geom_smooth(se=FALSE)
```
...
```{r}
png('in_person_binned_choice_rt.png', res=120, width=600, height=450)
dg <- d_trim %>%
  group_by(RTbin, Support_bin) %>% 
  summarize(Choice.m = 
...
```
```{r}
plot_model(rt.mod7d, type='pred', terms = 'LeftMoral_zm')
```
...
```{r}
png('pilot_ValueMCMCs.png', width=800, height=800)
par(mfrow=c(2,2))
plot(NULL, xlim=c(-6, 6), ylim=c(0, 1),
     xlab="Relative value of Left",
     ylab="% of Left",
...
```{r}
png('pilot_ValueMCMC.png', width=800, height=450)
par(mfrow=c(1,2))
plot(NULL, xlim=c(-6, 6), ylim=c(0, 1),
     xlab="Relative value of Left",
     ylab="% of Left",
...
```{r}
fit <- rstan::stan(file = '../../scratch-foo/choice_linearvalue_ddm.stan', 
