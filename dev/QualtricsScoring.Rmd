...
# Import data
```{r}
d_raw <- read_xlsx('../../scratch-foo/data/moral_choice/batch_5.xlsx',
               skip=1)
d_raw <- d_raw %>%
...
```
```{r}
bbmle::AICtab(resp_mod, resp_mod2, resp_mod3, resp_mod4,
      resp_mod5, resp_mod6)
```
```{r}
summary(resp_mod3)
...
```{r}
rt.mod2 <- lmer(log(RTs) ~ poly(LeftRDV, 2) + (1|SID),
                data=d_trim)
```
